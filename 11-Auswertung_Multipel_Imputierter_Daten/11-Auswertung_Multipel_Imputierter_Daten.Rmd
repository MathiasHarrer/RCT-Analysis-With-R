...
# Klinisch Signifikante & Reliable Veränderung

## Klinisch Signifikante & Reliable Veränderung (I)
...
## Klinisch Signifikante & Reliable Veränderung (II) {.t}
...
## Klinisch Signifikante & Reliable Veränderung (III) {.t}
...
## Klinisch Signifikante & Reliable Veränderung (IV) {.t}
...
## Generalisierte Linear Modelle (II) {.t}

**\alert{Die Struktur von GLMs}** \scriptsize [@dunn2018generalized, Kapitel 5] \par \normalsize


- GLMs sind eine **Verallgemeinerung** linearer Modelle auf die gesamte Klasse der **Exponentialfamilie** von Verteilungen ("exponential dispersion model family"; EDM). 

- Damit können auch Endpunkte wie **Zähl-/Eventdaten** oder **dichotome Outcomes** modelliert werden.

- Einfache lineare Modelle können direkt durch Matrixalgebra berechnet werden (**Ordinary Least Squares**; OLS). Bei (anderen) GLMs muss die **Optimierung iterativ** durch **Maximum-Likelihood-Schätzung** (MLE) **vorgenommen werden**. 


## Generalisierte Linear Modelle (III) {.t}

**\alert{Die Struktur von GLMs}** \scriptsize [@dunn2018generalized, Kapitel 5] \par \normalsize

\footnotesize
GLMs bestehen aus \underline{zwei} Komponenten: dem **linearen Prädiktor** $\eta_i~$...

$$\eta_i = (o_i) +\alpha + \mathbf{x}_i^\top\mathbf{\beta}$$

... und einer **Kopplungsfunktion** ("Linkfunktion"):

$$g(\mu_i) = \eta_i $$

Die inverse Kopplungsfunktion **"übersetzt"** dabei die Werte des **linearen Prädiktors** in das **Format der Responsevariable**: $g^{-1}(\eta_i) = \mu_i$; $\mu = E(y_i)$. 

In R können GLMs mit der `glm`-Funktion unter Angabe der Linkfunktion berechnet werden; z.B.

```{r, eval=F}
glm(y ~ trt + x, data = data, link = binomial("logit"))
```

\normalsize

## Generalisierte Linear Modelle (IV) {.t}

**\alert{Die Struktur von GLMs}** \scriptsize [@dunn2018generalized, Kapitel 5] \par \normalsize

\footnotesize

$$\eta_i = (\alert{o_i}) +\alpha + \mathbf{x}_i^\top\mathbf{\beta}$$
Der Term $o_i$ steht hier für einen **Offset**, der v.a. bei Poisson- und negativ binomialen Modellen verwendet wird. Offsets sind _a priori_ bekannte Werte, für die **keine Parameterschätzung** notwendig ist; in der Biomedizin oft die Dauer der Exposition $E$ bei der Analyse klinisch relevanter Events (z.B. Schlaganfälle während variierenden Zeitraums $E$). 

\underline{Für das Poisson-Modell ergibt sich so z.B.:}


\begin{align*}
\begin{split}
g(\mu) &= \overbrace{\text{log}_e(\mu)}^{\text{Linkfunktion}} = \eta \\
g(\mu) &= \text{log}_e\left(\frac{\mu}{E}\right) = \eta ~~~ \text{(mit Offset)}\\
&= \text{log}_e(\mu) = \eta + \text{log}_e(E)
\end{split}
\end{align*}

```{r, eval=F}
glm(y ~ trt + x + offset(E), data = data, link = poisson("log"))
```


\par \normalsize

## Generalisierte Linear Modelle (V) {.t}
...
data.frame(
  ...
  beta = c("$\\text{log}_e$(MD)", "OR", "RR", "HR", "(I)RR", "(I)RR", "-"),
